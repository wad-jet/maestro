...
- **Нативный permission-бастион OpenCode (R1+R4):** `/maestro-new` пишет в
  ...
  (`read`/`edit`) и 2-й эшелон для `bash`/`glob`/`grep` — fail-closed на уровне
  ядра OpenCode, не зависящий от плагина (Этап A; R2/R3 — Этап B после V1).
...
- **Fail-open без плагина — смягчён (Этап A).** При отключённом плагине файловая
  защита confidential **не полностью теряется**: нативный deny-baseline
  (`read`/`edit`/`bash`/`glob`/`grep`) остаётся в силе. **Sanitizer** и
  **enforcement плагина** (trusted-исключения по данным, access_policy) по-прежнему
  требуют плагина → P5-гейт сохраняется. R2/R3 (Этап B, после V1) переносят
  trusted-исключения нативно, снижая зависимость от плагина дальше.
