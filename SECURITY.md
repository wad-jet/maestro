...
### P8. Авто-режимы `@maestro-init` не ослабляют ИБ-контроль

Флаги `--auto-answer`/`--auto-ai` (режимы запуска, SKILL.md п. 3в) НЕ
распространяются на:
- **security-гейты:** 8.6 при `FINDINGS_FOUND` (включая HITL-заверение «sanitize
  пропустить» — P7), Security Review Точка 2, File access control, гейт плагина
  (P5). В auto-ai применяется гибрид только для **risk-reducing** дефолтов
  (вычистить и продолжить / запретить доступ) — не риск-принятие; принятие
  риска и стоп — HITL.
- **гейты 10 (spec, ⚑2) и 17 (pre-PR/merge, ⚑1)** — всегда HITL, в т.ч. в
  авто-режимах; в auto-ai — жёсткая пауза до ответа человека. Принятие спеки
  на гейте 10 — только явная команда.
- **чувствительные изменения (⚑4)** — внешние API-контракты интеграций и
  основополагающие правила бизнес-логики — только явное HITL-решение
  (sensitivity guard на гейте 12).

Механический пол не снимается: Ур.1 маскирование task-промптов, read-блоки
access_policy и confidential-deny исполняются плагином безусловно, в любом
режиме. Инварианты ⚑1–4 действуют во всех режимах (manual / auto-answer / auto-ai).
