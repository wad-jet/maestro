...
- `skills/maestro-benchmark/{SKILL.md,diff.mjs,sandbox-smoke.test.mjs,diff.test.mjs}` — `@maestro-benchmark` benchmark skill: фиксированное задание в песочнице (`maestro-sandbox.sh --benchmark`), отчёт прогона в `.maestro/benchmark-reports/` (`.md` + `.json`), детерминированная свёрка с прошлыми прогонами (`diff.mjs`, 0 LLM).
...
**Локальный dev-зеркал `.opencode/` (gitignored, dogfooding) — условная гигиена.** Для доставки не требуется (см. выше), но если используется — синхронизируется **только целиком** через `agpack sync` после push в remote (источник — remote, не локальное дерево); частичные ручные правки запрещены. При переименованиях (скиллы/команды/агенты/пути): push → `agpack sync` → grep остатков старого имени по всей поверхности (skills/, commands/, agents/, manual_docs/, скрипты) + удаление stale-каталогов (agpack старые имена не удаляет).
...
- Global observability (not bound to any agent): logs key events for all sessions — `session.error`, `task` dispatch (`tool.execute.before`/`tool.execute.after`, sanitized title), empty subagent result. Sanitizes `task` prompts (Level 1 Security Review) before untrusted subagents. The old bootstrap-directive injection via `experimental.chat.messages.transform` was removed — `transform` must stay `undefined`.
