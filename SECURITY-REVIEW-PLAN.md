...
>
> **Update 2026-08-19:** конфиги консолидированы в `maestro.json` (секции
> `trust`, `access_policy`, `sanitizer_whitelist`). Ссылки на
> `trust-config.json`, `.maestro/access-policy.json`,
> `.maestro/sanitizer-whitelist.json` ниже — исторические (backward compat
> поддерживается плагином). Текущая документация — в `skills/maestro/SKILL.md`.
...
2. **Trust:** сабагент `sanitizer` — trusted, read-only, единственный кому
   разрешено видеть сырые данные (чтобы пометить). **Дополнительно:** сабагент
   `design` (spec formation, шаг 8) — trusted, видит полный контекст
   (user story + project context) для качественного spec. Оба trusted
   перечислены в `trust-config.json`.
...
- `design` сабагент (spec formation, шаг 8) — trusted, видит полный контекст
  для качественного spec; его промпт при диспатче не санизируется.
