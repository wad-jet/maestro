...
- **Версионирование:** **да** (обязательно учитывать при каждой фиче/фиксе/
  ...
  - **Схема:** semver `MAJOR.MINOR.PATCH`. Текущая версия дистрибутива —
    `4.12.0` (единая для скиллов и плагина, см. `manual_docs/how-to/update-maestro.md`).
  ...
  - **Действия при релизе:** changelog `manual_docs/overview/changelog.md` —
    ...
    выполняется после merge на base-ветке (до push, если push ещё не выполнен).
