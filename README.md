...
- **Рекомендуемый способ — `maestro-update.sh`:** одна команда определяет целевую
  версию, запускает `agpack sync`, очищает кэш плагина; опционально `--pin <sha>`. перед обновлением скрипт сам проверяет собственную актуальность и при расхождении предлагает самообновиться (интерактивно).
