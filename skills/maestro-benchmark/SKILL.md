...
### Шаг 0. Предварительный guard

Фикстура должна быть нетронута: `git -C .sandbox rev-list --all --count` == 1
и чистое `git status --porcelain`. Иначе — **стоп** с пояснением: песочница
сброшена, отчёт по завершённой сессии недостоверен (артефакты прогона
отсутствуют).
