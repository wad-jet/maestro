...
3в. **Режимы запуска (авто-режимы):**
   ...
   **auto-ai: security-усиления (R1–R4):**
   ...
   - **Механический пол:** Ур.1 маскирование task-промптов, read-блоки нативного
     permission-слоя и confidential-deny исполняются плагином безусловно —
...
|---|---|---|
| **trusted** (`true` в `maestro.json`) | **skip** | по нативным permissions opencode (без ограничений) |
...
- **Этап 2 (сделан):** в плагине `maestro-bootstrap` реализованы Уровень 1
  ...
  (Уровень 2) и генерирует/поддерживает maestro.json.
