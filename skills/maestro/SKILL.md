...
🟢 11. [agent] writing-plans -> Implementation Plan
      **Fast-track (шаг 7d):** план пишется ИЗ внешнего spec (шаг 8 пропущен).
      ...
      - **Self-check плана (обязателен перед plan-gate, 4 проверки):**
        ...
        (3) каждый файл-цель существует (`test -f <path>` для каждого
        Create/Modify);
      ...
      - **Task-review сверка spec-инвариантов:** при диспатче task-reviewer
        ключевых задач (шаг 13) включать в Global constraints явные
        spec-инварианты задачи (цепочки, timeout/guard, enum-консистентность)
