...
**Быстрый триггер (fast trigger).** Если итоговый диф можно описать одним
предложением, фича классифицируется как простая (Bounded) без применения
матрицы. Триггер НЕ переопределяет сигналы колонки «Архитектурная» и строку
«Public API»: если есть хотя бы один такой сигнал, применяется полная матрица.
...
🟡  0. [agent] Load Project Context
      ...
      — Если файла нет: HITL-диалог для создания по 14 категориям
        ...
        см. `skills/maestro-setup/init-context.md`; для нового проекта рекомендуется
        команда `/maestro-setup` (создаёт context + конфиг; дизайн/scaffold/roadmap — `/maestro-design`).
...
🟡  7. -- HITL GATE: категория фичи (см. Feature Classification) --
      ВАРИАНТЫ:
        ...
        (b) Простая — по **Bounded-логике** (OQ-6). **Fast trigger:** если
            итоговый диф можно описать одним предложением — классификация
            простая/Bounded без обсуждения (кроме сигналов колонки
            «Архитектурная» — см. Feature Classification). Короткий дизайн в
            чате → approval → SDD. Шаги 8-10 пропускаются (no formal spec). Вместо
...
🟢  8.5. [agent] Оценка изменений контекста
       ...
       — **Гигиена контекста:** при применении pending context changes
          (шаг 12a) следовать правилам гигиены project-context.md (тест
          обрезки, ≤1 IMPORTANT-акцент на раздел, редко-нужные знания по
          требованию) — канон правил: скилл `maestro-setup` (Задача 2).
...
🟡 17. -- HITL GATE: pre-PR --
      ...
      **и список открытых issues (open + follow-up) с severity** из шага 16.
      ...
      в текущей сессии — оркестратор выполняет их до гейта; исключение —
      осознанный пропуск на шаге 15 (условный пропуск: багфикс probe==fix или
      low-risk фича ≤2 файла): вместо вывода гейт показывает «Тесты
      пропущены на шаге 15 (low-risk), coverage = <N%>» (решение шага 15
      не аннулируется; если fix-loop менял код — пропуск аннулируется
      правилом шага 15).
...
Метаданные о пройденном ревью/санизации, встроенные в spec файл. Являются
**provenance-рекомендацией** для HITL-решений на fast-track (шаг 7d) и
re-entry (повторный запуск `maestro` на том же spec), а не основанием
авто-пропуска шагов (см. правило 9).
