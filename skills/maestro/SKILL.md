...
🟡 12. -- HITL GATE: plan утверждён --
      **Авто-режимы:** auto-answer — авто (a) Approve при выполнении P1.3
      ...
      изменениях ⚑4 — HITL. Гейт 12 не является инвариантным стопом: последующие
...
3в. **Режимы запуска (авто-режимы):**
   ...
   **Никогда не авто (все режимы):** гейты 10 (⚑2 — финальное ревью спеки только
   разработчик, принятие — только явная команда) и 17 (⚑1 — PR/мерж только
   человек); контентные вопросы фичи (brainstorm Q/A, custodian-сходимость, имя
   ветки при неоднозначности, D2/D6/D7, spike-гейты); security-гейты (8.6 при
   `FINDINGS_FOUND`, включая HITL-заверение «sanitize пропустить» — P7; Точка 2;
   File access control; гейт плагина P5); плато и особый случай Revise (шаг 10);
   Tier 3 и упавшие команды (15/15a); docs-расхождения (14); reconciliation (13f);
   чувствительные изменения (⚑4).
   В auto-ai security-гейты — гибрид: безопасный дефолт ИИ применяет сам
   («вычистить и продолжить»); принятие риска и стоп — HITL. Правило направления:
...
| **Chain-approval: принимать "continue" за approval всех последующих gates** | Gate X+1 не имеет ответа пользователя. Каждый gate — отдельный вопрос, "continue" переводит только на следующий шаг. **Единственное исключение — явные флаги авто-режимов `--auto-answer`/`--auto-ai`** (п. 3в): они дают предварительное согласие на весь прогон, но только на гейты из перечня 3в; гейты 10/17, security-гейты, контентные вопросы и чувствительные изменения (⚑1–4) остаются HITL. |
