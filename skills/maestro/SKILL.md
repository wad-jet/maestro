...
**Инварианты автоматизации (действуют во всех режимах):** ⚑1 PR/мерж — только
человек (гейт 17); ⚑2 финальное ревью спеки — только человек, принятие спеки —
только явная команда (гейт 10); ⚑3 ИИ-тесты не полностью доверены (результаты
показывает человек на гейте 17); ⚑4 внешние API-контракты интеграций и
...
🟡 17. -- HITL GATE: pre-PR --
      **Всегда HITL (⚑1 — PR/мерж только человек, лично или явным поручением),
...
3в. **Режимы запуска (авто-режимы):**
   ...
   **Никогда не авто (все режимы, включая auto-ai):** гейты 10 (⚑2 — финальное
   ревью спеки только разработчик, принятие — только явная команда) и 17 (⚑1 —
   PR/мерж только человек); плато и особый случай Revise (шаг 10); Tier 3 и
   ...
   **auto-ai: механика ralph loop.** Режим ведётся циклом ralph loop:
   ...
   - **Паузы на гейтах 10/17:** цикл НЕ форсирует проход инвариантных гейтов — на
     ...
     после ответа цикл продолжается.
...
decision-диспатч (`opus` для D2, `sonnet` для D6); D7 (переход к фиксу) — следует из
D6, в auto-ai авто. В auto-answer D2/D6/D7 — всегда HITL. Систематический дебаг
...
spec-уровневые). **Эскалация:** sonnet вернулся `low`/`HITL_REQUIRED` → повторный
диспатч `opus`. **Fallback:** opus тоже неуверен → консервативный выбор (более
