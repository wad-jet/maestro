...
- [ ] **Production-путь в тестах:** для публичного «шва» модуля (событие, хук,
  регистрация конструктора) — минимум один интеграционный тест через
  production-конструктор (реальный путь сборки, БЕЗ ручной подстановки
  `deps.*`/логгера). Тест только через инъекцию зависимостей не считается
  покрытием — не ловит «событие реализовано, но не подключено» (wiring-gap).
