...
Версия плагина `maestro-bootstrap` (`package.json` → `version`, сейчас `1.1.0`)
нигде не экспонируется в рантайме. При диагностике нельзя понять, какая версия
реально загружена в сессии OpenCode, нет версии в аудит-логе, и нет способа
сообщить пользователю активную версию. OpenCode не даёт нативного способа
запросить версию загруженного плагина (`opencode debug config` показывает только
plugin-спецификацию, для npm-установки версия в спеке отсутствует).
...
(`plugin initialized`). Единый источник версии — `package.json` плагина.
...
- Резолвит путь к `package.json` через `import.meta.url` относительно `core.js`
  (надёжно и для npm-установки, и для локального пути — package.json лежит рядом).
