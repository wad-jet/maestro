...
> Статус: **реализовано** (см. `skills/maestro-init/SKILL.md`, задачи 3/3а/4/5;
> дизайн/scaffold/roadmap вынесены в `/maestro-design` — см. maestro-init-tasks-plan).
...
> файлы удалены из поддержки, тесты 72/72, пример `maestro.example.json` создан.
> Нумерация «Шаг 1/1.5» ниже устарела → в SKILL.md используются «Задача 2/3/3а»;
> конфиги реализованы как «Задача 3» (+3а каталоги) в `skills/maestro-init/SKILL.md`;
> дизайн/scaffold/roadmap (ранее (a)/(b)/(c)) — в `/maestro-design`.
...
**Текущий разрыв (на момент написания):** `/maestro-init` создавал только
`docs/project-context.md`, spec, scaffold и `docs/roadmap.md`. Конфиги, которые
мастер-система ожидает (см. плагин `maestro-bootstrap`,
`skills/maestro/SKILL.md`), **не генерировались**. (Сейчас spec/scaffold/roadmap
вынесены в `/maestro-design` — см. статус выше.)
...
## Новый шаг в `skills/maestro-init/SKILL.md`: «Задача 3. Конфигурация maestro»

Реализован как «Задача 3» (+3а каталоги) в `skills/maestro-init/SKILL.md`
(в черновике назывался «Шаг 1.5»). Размещается между «Задача 2. Сбор контекста»
и завершением setup; дизайн/scaffold/roadmap (ранее (a)/(b)/(c)) вынесены
в отдельную команду `/maestro-design`:
...
**C1 (критично):** Задача 3 выполняется в **обоих** путях предусловия 2:
- Новый проект (файла нет): Задача 2 (сбор контекста) → **Задача 3**
- Существующий проект (option a): загрузка контекста из файла → **Задача 3**
...
**Секция `trust`** — из §3 Стек + §5 Домены/модули + §12 Безопасность:
```json
"trust": {
  "design": true,
  "sanitizer": true
...
- `node --test plugins/maestro-bootstrap/index.test.js` — 72/72 (плагин не
...
**Проблема (на момент написания):** Предусловие 2 (проверка
`docs/project-context.md`) option (a): «перечитать/восстановить контекст из
него и перейти сразу к завершению» — пропускало «Задачу 2 (сбор контекста)».
Конфиг-генерация (тогда «Шаг 1.5», сейчас «Задача 3») размещалась ПОСЛЕ сбора
контекста → могла быть пропущена при option (a). Это случай повторного запуска
(идемпотентности): существующий проект, пользователь выбирает (a).

**Решение:** конфиг-генерация выполняется в **обоих** путях:
- Новый проект: Задача 2 (сбор контекста) → **Задача 3** (конфиги)
- Существующий проект (option a): загрузка контекста из файла → **Задача 3**

Реализация в `skills/maestro-init/SKILL.md`: задача 3 выполняется после
предусловий любым путём; дизайн/scaffold/roadmap — отдельно в `/maestro-design`.
...
**Все решения учтены в обновлённом плане (см. выше). Реализация выполнена — см. статус вверху: конфиги в `skills/maestro-init/SKILL.md` (Задача 3), дизайн/scaffold/roadmap в `/maestro-design`.**
