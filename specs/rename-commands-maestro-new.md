...
- `package.json`: 1.2.2 → **2.0.0** (по semver: breaking rename команд — мажорный bump; разрыв совместимости зафиксирован корректно).
- `maestro.json`: `expected_version` **убрать совсем** — версия дистрибутива не хранится в конфиге проекта (конфиг описывает проект, а не установку). `/maestro-version` показывает фактическую версию плагина из `.maestro/plugin-version`; плагин не зеркалирует `expected-version` и не предупреждает о рассинхроне.
