...
- `maestro.json`: `expected_version` **НЕ менять** — остаётся 1.2.2 (фактически установленная версия; `maestro-update.sh` не запускался). Актуализируется только самим скриптом при его запуске.
