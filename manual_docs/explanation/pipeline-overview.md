...
Пользователь вызывает `/maestro-init` с описанием задачи. Оркестратор загружает
...
| Шаг | Действие | HITL? |
|---|---|---|
| D1 | Systematic-debugging: ресерч кода, логов, воспроизведение. **Memory layer:** перед гипотезами — `memory_search` по симптому/модулям (обязательно; память не включена → молча пропуск) | — |
...
`@maestro-init` поддерживает три режима (per-run флаг): **manual** (все гейты
вручную, по умолчанию), **auto-answer** (`--auto-answer`/`-aa` — полный флоу,
рекомендации на рутинных гейтах принимаются автоматически с уведомлением;
гейты 10/17 и security-гейты остаются ручными), **auto-ai** (`--auto-ai`/`-ai` —
ИИ сам принимает решения с доп. анализом через decision-диспатч sonnet/opus,
механика — ralph loop; гейты 10/17 и чувствительные изменения — всегда вручную).
Инварианты
⚑1–4 (PR/мерж, финальное ревью спеки, доверие к ИИ-тестам, чувствительные
контракты/бизнес-логика) действуют во всех режимах. Подробнее —
[HITL-гейты](../reference/hitl-gates.md).
