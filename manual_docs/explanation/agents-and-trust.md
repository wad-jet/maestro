...
- **Prompt-injection через память — остаточный риск.** Память — исторический
  ...
  блок `## Контекст из памяти maestro` и результат `memory_search` содержат
...
- **Экспорт/импорт — локальная граница (v2).** `memory_export` пишет JSONL
  полной схемы v1 (включая embedding). Путь по умолчанию — **локальный**
...
- **Кросс-проектный поиск — opt-in.** `memory_search` с `project` ищет по
  записям других проектов **только когда задан явно** (не default); данные
  маскированы; доступен только для централизованных бэкендов (qdrant/pgvector).
  Прецедент — `namespace` (осознанный шаринг); остаточный риск prompt-injection
  сохраняется (mitigated framing из v1).
- **Локальность.** Эмбеддинги — в процессе, БД — на диске, кэш модели —
  локальный. Единственный сетевой вызов — однократная загрузка модели
  эмбеддингов (offline-режим с предзагрузкой). `memory.db` вне git (глобальный
