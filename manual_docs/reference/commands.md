...
Читает `.maestro/plugin-version` (пишется при инициализации плагина) и сравнивает
с ожидаемой версией из `.maestro/expected-version` — при рассинхроне показывает
предупреждение. Если файла нет — сообщает, что плагин не инициализирован или
версия неизвестна (признак сбоя init плагина).
