...
токены (primary-сессия + по сабагентам через child-сессии), cost (— при
отсутствии прайсинга), активное время (wall − user_wait), HITL-гейты,
retry, циклы ревью (механический ориентир + нарратив); методология Effort —
