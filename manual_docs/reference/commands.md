...
канон — `skills/maestro/invariants.md`). После генерации
...
`.maestro/memory-report-<YYYYMMDD-HHMMSS>.html` с авто-preview в браузере (отключается `memory.report.preview: false`). При отсутствии секции `Узлы графа (M):` в выводе `memory_stats_detail` (устаревший плагин) — предупреждение и упрощённый session-level граф. Подробнее —
[Память](../reference/memory.md).
