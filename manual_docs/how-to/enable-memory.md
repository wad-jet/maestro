...
**Зачем включать:** память — индекс прошлых сессий, а спеки/планы в
`docs/superpowers/{specs,plans}/` — источник истины для контекста фичи.
Поле `artifacts[]` записей (v5.2) связывает recall-хит с конкретным
файлом спеки/плана — один `read` вместо перебора каталога. **Эффект растёт
с масштабом:** для больших проектов с большой историей и большим числом
...
- **Только агрегаты (SEC-4b):** при `memory.report.include_text: false` (default)
  ...
  в графе.
- **Guard полноты данных:** команда проверяет наличие секции `Узлы графа (M):`
  в выводе `memory_stats_detail`. Если её нет (работающий плагин старше фичи
  commit-графа) — выводится предупреждение и отчёт рендерится в упрощённом
  session-level формате, а не молча в неверном.
...
что у recall (единый гибридный код-путь: embed + FTS-нога → RRF → пост-фильтр
`min_score`), возвращает top-k записей со скорами, автором, датой, проектом.
Назначение — подбор `top_k`/`min_score` без угадывания:
...
| `memory_search` возвращает «Ничего не найдено» | Память пуста (backfill ещё не прошёл) или запрос ниже порога `min_score`. Строка результата несёт порог и scope: «Ничего не найдено в памяти (порог min_score X, scope Y)». FTS-only хиты (display-score 0.5) не показываются при `min_score` > 0.5 |
...
| `min_score` | Записи есть, но ниже порога `min_score` — снизьте порог или проверьте релевантность записей. FTS-only хиты (display-score 0.5) не показываются при `min_score` > 0.5 |
