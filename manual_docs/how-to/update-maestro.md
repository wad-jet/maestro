...
репозитория (например, `3.0.0`). Скиллы отдельной версии не имеют; их версия —
...
> репозитория: скрипт при каждом запуске сам проверяет собственную актуальность (см. «Самопроверка версии скрипта» ниже).
> ```bash
...
Порядок обязателен: **обновить (скиллы Шаг 1; плагин Шаг 2 — с очисткой кэша) →
перезапустить → проверить**.
...
3. Сверьте с версией из кэша плагина (`node_modules/maestro-bootstrap/package.json`)
   (это версия и скилов, и плагина). Расхождение → скилы/плагин не той версии.
4. Если команда сообщает «не инициализирован или версия неизвестна» — плагин не
   загрузился / сбой init.

### Проверка после перезапуска (smoke-чеклист)

После обновления и перезапуска OpenCode проверьте:
...
**Rollout-порядок для релизящего (при публикации новой версии maestro):**
1. **Push** новой версии в remote-репозиторий (wad-jet/maestro).
2. **`agpack sync`** (обновить скилы/команды/агенты из remote).
3. **Очистить кэш плагина** (`maestro-update.sh` или вручную).
4. **Перезапустить** OpenCode.
5. Проверить: `/maestro-version` = новая версия; smoke-чеклист выше.

Порядок нарушать нельзя: **push ДО очистки кэша/перезапуска** — иначе кэш подтянет
старую версию и перезапуск будет потерян (инцидент 09-09).
