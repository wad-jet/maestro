# Настройка проекта для maestro

[Назад к оглавлению](../index.md)

Это обучающий материал: пошаговая подготовка проекта к использованию скилла
`maestro` перед запуском первых фич. Покрывает оба случая: **новый проект** и
**существующий проект, где maestro ранее не применялся**.

## 🎯 Что вы изучите

- Разницу между командами setup: `/maestro-new`, `/maestro-design` и `/maestro-init`.
- Как подготовить контекст и конфигурацию проекта.
- Как настроить модели агентов по тирам (и как их определяет `/maestro-new`).
- Как перейти к работе над фичами.

## 🧭 Три команды: порядок

| Команда | Что делает | Когда |
|---|---|---|
| `/maestro-new` | Setup: `project-context.md` (14 категорий), конфигурация (`maestro.json`, `.gitignore`, плагин+модели в `.opencode/opencode.json`/global), каталоги, проверки | Первый шаг — настроить проект |
| `/maestro-design` | Дизайн + spec, scaffold (каркас кода), roadmap | После init — спроектировать архитектуру |
| `/maestro-init` | Оркестрация фич/багфиксов (design → spec → plan → SDD → review) | Когда проект готов |

**Порядок:** `/maestro-new` → `/maestro-design` (для нового проекта) → `/maestro-init`.

---

## Вариант A — Новый проект

### Шаг 1: `/maestro-new`

Запустите `/maestro-new` в любой primary-сессии. Скилл `maestro-new` выполнит:

1. **Предусловие `AGENTS.md`** — если файла нет, предложит выполнить встроенный
   `/init` opencode (создаёт `AGENTS.md`).
2. **Сбор контекста** — интерактивный опрос по 14 категориям → создаёт
   `docs/project-context.md`. Обязательные секции: 1, 2, 3, 4, 9, 14.
3. **Конфигурация** — генерирует (по канону скилла `maestro-assistant`):
   - `maestro.json` — `trust` (custodian, sanitizer), `access_policy`, `confidential`,
     `sanitizer_whitelist`;
   - плагин `maestro-bootstrap` + модели агентов — в `.opencode/opencode.json`
     (реком.) или глобальном конфиге;
   - `.gitignore` — весь `.maestro/` и `.opencode/`;
   - `regression/` — структуру реестра рисков.
4. **Каталоги** — `.maestro/`, `docs/superpowers/{specs,plans}/`.
5. **Проверки** — скилы superpowers (предложит установить через HITL), плагин
   `maestro-bootstrap` (не блокер).

> Полный JSON-канон `maestro.json` и правила вывода секций — в скилле `maestro-assistant`
> (`skills/maestro-assistant/SKILL.md`). Настройка конфигурации/структуры/контекста после
> setup — через `/maestro-assistant`.

> На шаге «Настройка моделей» `/maestro-new` спросит модели для агентов — см.
> раздел [Настройка моделей по тирам](#настройка-моделей-по-тирам) ниже.

### Шаг 2: `/maestro-design`

После init запустите `/maestro-design`. Скилл `maestro-design` выполнит:
- **(a) Дизайн + spec** — primary (brainstorming) формулирует spec, опираясь на
  Q/A по confidential от trusted-сабагента `custodian` (возвращает только
  агрегаты, без значений), результат — `docs/superpowers/specs/YYYY-MM-DD-<project>-design.md`.
  Опционально — Spec Review (`opus`).
- **(b) Scaffold** — создаст каркас кода (структура каталогов, роуты, DTO,
  конфиги) через `implementer-prompt.md` (TDD) с диспатчем `haiku`/`sonnet`.
- **(c) Roadmap** — создаст `docs/roadmap.md` (MVP + этапы развития).

### Шаг 3: `/maestro-init`

Теперь проект готов. Используйте `/maestro-init` для фич/багфиксов (см.
[Быстрый старт](../overview/quick-start.md) и
[Запуск первой фичи](run-first-feature.md)).

---

## Вариант B — Существующий проект (maestro ранее не применялся)

### Особенности

- `docs/project-context.md` может уже существовать → `/maestro-new` предложит
  перечитать его (а не опрашивать заново) или пересоздать.
- Конфигурации maestro может не быть → `/maestro-new` создаст её **идемпотентно**:
  diff-merge, **не перезаписывая** пользовательские правки.
- Модели агентов — подсказки берутся из **текущего** `.opencode/opencode.json` или
  глобального конфига (см. раздел про модели).

### Шаг 1: `/maestro-new`

Запустите `/maestro-new`. Поведение:

1. **`project-context.md`** — если существует, HITL: (a) перечитать и перейти к
   конфигурации / (b) пересоздать с нуля / (c) отмена.
2. **Конфигурация** — если `maestro.json` есть, покажет diff «желаемое vs
   текущее» по секциям (`trust`, `access_policy`, `sanitizer_whitelist`) и
   предложит: обновить / пропустить / отмена. **Пользовательские правки
   сохраняются** (merge).
3. **Модели агентов** — для каждого из 7 агентов предложение формируется из:
   1. текущий `.opencode/opencode.json` (project);
   2. global (`~/.config/opencode/opencode.json`);
   3. tier-подсказка (для новых/не настроенных).
4. **Каталоги и проверки** — как в новом проекте.

### Шаг 2: `/maestro-design`

`/maestro-design` проверяет, выполнялся ли `/maestro-new` (Признаки:
`docs/project-context.md`, `maestro.json`, `.maestro/last-run.md`). Если init не
выполнялся — предложит выполнить его сначала. Затем — дизайн + spec + scaffold +
roadmap, как в Варианте A.

### Шаг 3: `/maestro-init`

Проект готов. Используйте `/maestro-init`.

---

## Настройка моделей по тирам

### Оси Tier и Trust — ортогональны

- **Tier** — класс ответственности агента (тип задачи, модальность, требование к
  рассуждению). «Мощность» — лишь косвенное следствие (opus ≥ sonnet ≥ haiku по
  способности к рассуждению), не единственный критерий: `fable` — модальность
  (примеры/метафоры), `sanitizer` — доверие, не мощность. Контекст задаёт
  конкретная модель, не тир.
- **Trust** — доверие (`maestro.json → trust`), доступ к секретам. **Trusted — это
  атрибут безопасности, а не мощность.**

### Агенты: tier и trust

| Агент | Tier (роль) | Trusted? |
|---|---|---|
| `custodian` | opus (confidential Q/A broker) | ✅ |
| `sanitizer` | своя модель (security review) | ✅ |
| `opus` | opus (spec review) | ❌ |
| `code-reviewer` | opus (code review) | ❌ |
| `haiku` | haiku (механические задачи) | ❌ |
| `sonnet` | sonnet (интеграционные задачи) | ❌ |
| `fable` | fable (примеры/метафоры) | ❌ |

> `custodian` и `sanitizer` — **оба trusted**, но **разные агенты**. Модели могут быть
> разными, но **одна модель тоже допустима** на усмотрение пользователя (например,
> одна локальная/изолированная для обоих).

### Tier → тип задач / ответственность → сабагент

| Tier | Тип задач / ответственность | Требуемое | Сабагент |
|---|---|---|---|
| haiku | механические: 1-2 файла, трансляция, юнит-тесты | низкий reasoning, следование spec | `haiku` |
| sonnet | интеграционные: multi-file, отладка, pattern matching | средний reasoning + контекст | `sonnet` |
| opus | архитектура, spec, дизайн-решения, ревью | высокий reasoning/дизайн-суждение | `custodian` (confidential Q/A, **trusted**), `opus`-агент (spec review, untrusted), `code-reviewer` (code review, untrusted) |
| fable | примеры, метафоры, пояснения | модальность, не мощность | `fable` |
| своя (sanitizer) | security-пометка: поиск/маркировка чувствительных данных | **вне tier-класса**: точность + доверие (trusted), не мощность | `sanitizer` |

> **Тир `opus` и агент `opus` — совпадение имён.** Тир — класс задач; агент `opus` —
> конкретный сабагент этого тира (spec review). Не путать.

> **«Своя модель»** (`sanitizer`) — вне четырёх тиров, подбирается под
> security-задачу (точность, надёжность), не обязана быть мощной/лёгкой;
> `temperature 0.0` по умолчанию. Определение — в [Выборе моделей](../reference/model-selection.md).

> Без явного выбора tier сабагент наследует дорогую модель сессии — это разрушает
> экономику tier-выбора. Всегда диспатчить сабагента нужного тира.

### Как `/maestro-new` определяет доступные модели (D2)

Список кандидатов для tier-подсказок `/maestro-new` определяет так:

1. **Основной источник — `opencode models <provider>`** (запрос к рантайму opencode,
   не чтение глобального файла). Выполняется для каждого известного провайдера
   (`provider.*` в merge-конфиге), списки объединяются.
2. **Fallback** (если команда недоступна / ошибка / пустой список) — модели из
   `provider.<name>.models` в merge-конфиге:

   | Уровень | Файл |
   |---|---|
   | global | `~/.config/opencode/opencode.json` |
   | project | `<project>/.opencode/opencode.json` |

   Приоритет merge: project > global.
3. **Ручной ввод** — если кандидатов нет: HITL-ввод ID вручную + попытка
   `opencode models <provider>`.

> Это устраняет «странное» обращение к глобальному файлу при настройке opencode:
> доступные модели берутся из рантайма, а не «прогуливанием» конфигов вручную.

### Как выбираются модели (M1) — 7 HITL-вопросов

Для **каждого из 7 агентов** `/maestro-new` задаёт отдельный вопрос —
выбор из вариантов через вопросный инструмент: кандидаты моделей (из D2) /
`auto` / «оставить текущую» (если агент уже настроен), плюс «свой вариант»
(ручной ввод ID модели).

- **7 отдельных вопросов** — по одному на каждого агента (гибкость выбора):
  `custodian` и `sanitizer` могут получить разные модели, но **одна модель тоже
  допустима** (см. выше).
- **`auto`** — ключ `model` не пишется (наследуется дефолт сессии).
- **Плейсхолдеры запрещены** (`<...>` невалидны, сломают загрузку агента).
- **Учёт global-агентов.** М1 читает `agent.<name>.model` в merge-представлении:
  project `.opencode/opencode.json` → global (`~/.config/opencode/opencode.json`).
  «Оставить текущую» — «из проекта» или «из global» в зависимости от источника.
  При «оставить из global» в проект ничего не пишется.

#### Вариант: глобальная настройка по тирам (рекомендуется)

Настроить `agent.{custodian,haiku,sonnet,opus,fable,code-reviewer,sanitizer}`
(model + temperature) один раз в `~/.config/opencode/opencode.json` — новые
проекты наследуют значения через merge-конфигурацию OpenCode.

```jsonc
// ~/.config/opencode/opencode.json
{
  "agent": {
    "haiku": { "model": "akash/Qwen/Qwen3.6-35B-A3B", "temperature": 0.0 },
    "sonnet": { "model": "akash/deepseek-ai/DeepSeek-V4-Flash", "temperature": 0.1 },
    "opus": { "model": "akash/zai-org/GLM-5.2", "temperature": 0.1 },
    "code-reviewer": { "model": "akash/deepseek-ai/DeepSeek-V4-Flash-0731", "temperature": 0.2 },
    "fable": { "model": "akash/deepseek-ai/DeepSeek-V4-Flash", "temperature": 0.7 },
    "custodian": { "model": "akash/Qwen/Qwen3.6-35B-A3B", "temperature": 0.1 },
    "sanitizer": { "model": "akash/Qwen/Qwen3.6-35B-A3B", "temperature": 0.0 }
  }
}
```

При `/maestro-new` новый проект (без локальных моделей) получит «оставить
текущую (из global)» первым вариантом — наследование без вопросов. Project
`.opencode/opencode.json` переопределяет global (merge-приоритет project > global),
если нужен индивидуальный набор.

---

### Пример mapping tier → модель

В проекте `maestro` модели настроены так (пример):

| Агент | Tier | Модель (пример) | Темп. |
|---|---|---|---|
| `haiku` | haiku | `akash/Qwen/Qwen3.6-35B-A3B` | 0.0 |
| `sonnet` | sonnet | `akash/deepseek-ai/DeepSeek-V4-Flash` | 0.1 |
| `opus` | opus | `akash/zai-org/GLM-5.2` | 0.1 |
| `code-reviewer` | opus | `akash/deepseek-ai/DeepSeek-V4-Flash-0731` | 0.2 |
| `fable` | fable | `akash/deepseek-ai/DeepSeek-V4-Flash` | 0.7 |
| `custodian` | opus | (модель opus-tier) | 0.1 |
| `sanitizer` | своя | (безопасная/дефолтная) | 0.0 |

`temperature` — **дефолт по tier**: `/maestro-new` проставляет его, если у агента
нет собственного значения в merge-конфиге (`.opencode/opencode.json` или global;
существующее не перезаписывает).
Полная таблица дефолтов — в [Выбор моделей](../reference/model-selection.md).

> Модели — **preference пользователя**. `/maestro-new` лишь предлагает кандидатов
> из доступных (D2) и tier-класса; конкретный выбор — за пользователем.

### Как проверить конфигурацию моделей (тестирование)

После настройки моделей проверьте, что каждый сабагент реально диспатчится и
модель доступна. Для этого используйте команду `/test-agents`:

```bash
/test-agents
```

**Что делает проверка:**

1. **Диспатч всех 7 сабагентов** — `custodian`, `haiku`, `sonnet`, `opus`, `fable`,
   `code-reviewer`, `sanitizer` — через `task`-тул реальным диспатчем. Каждому
   агенту даётся одинаковая тривиальная задача: «Верни ровно одно слово OK и своё
   имя агента. Не используй инструменты».
2. **Сводная таблица статусов** — по каждому агенту `OK / FAIL`:
   - **OK** — диспатч успешен, сабагент вернул ответ;
   - **FAIL** — диспатч упал (недоступна/невалидная модель, ошибка провайдера,
     таймаут), в колонке «Причина» — текст ошибки.

3. **Проверка confidential-инварианта** (P1/P3, `SECURITY.md`) — доступ к
   `docs/confidential/**` не должен зависеть от доступности модели:
   - для каждого trusted-агента (`custodian`, `sanitizer`), если он в основной
     таблице `OK`, дать задачу «Прочитай `docs/confidential/<файл>.md`» — он должен
     прочитать (его право);
   - затем из primary-сессии попытаться прочитать тот же файл — должен прийти
     `confidential: deny`.

   Статусы: `PASS` (trusted прочитал, primary получил deny) / `FAIL` (утечка —
   СТОП) / `skipped` (нет `docs/confidential/**`).

**Ключевые правила:**

- Проверка **не читает конфиги** (`maestro.json`) и не сверяется с ними — валидируется
  только реальная работа модели через диспатч.
- Если trusted-модель недоступна (FAIL), confidential-инвариант воспроизводится
  только при фактической недоступности и детерминированно не тестируется
  (см. `SECURITY.md` → §6).

**Когда запускать:**

- Сразу после `/maestro-new` — убедиться, что выбранные модели валидны и все
  7 сабагентов отвечают.
- После изменения конфигурации моделей (вручную, через `/maestro-assistant`,
  либо смены провайдера/модели) — подтвердить, что ничего не сломалось.

Подробное описание команды — в [Справочнике команд](../reference/commands.md).

---

## Что дальше

- [Быстрый старт](../overview/quick-start.md) — минимальный путь к фиче.
- [Запуск первой фичи](run-first-feature.md) — полный цикл 0→18.
- [Выбор моделей](../reference/model-selection.md) — справочник tier/субагентов.

## 🔗 Связанные разделы

- [Выбор моделей](../reference/model-selection.md)
- [Команды](../reference/commands.md)
- [Агенты и модель доверия](../explanation/agents-and-trust.md)