...
- Разницу между командами setup: `/maestro-setup`, `/maestro-design` и `/maestro-init`.
- Как подготовить контекст и конфигурацию проекта.
- Как настроить модели агентов по тирам (и как их определяет `/maestro-setup`).
...
|---|---|---|
| `/maestro-setup` | Setup: `project-context.md` (14 категорий), конфигурация (`maestro.json`, `.gitignore`, плагин+модели в `.opencode/opencode.json`/global), каталоги, проверки | Первый шаг — настроить проект |
| `/maestro-design` | Дизайн + spec, scaffold (каркас кода), roadmap | После init — спроектировать архитектуру |
| `/maestro-init` | Оркестрация фич/багфиксов (design → spec → plan → SDD → review) | Когда проект готов |

**Порядок:** `/maestro-setup` → `/maestro-design` (для нового проекта) → `/maestro-init`.
...
Запустите `/maestro-setup` в любой primary-сессии. Скилл `maestro-setup` выполнит:
...
> На шаге «Настройка моделей» `/maestro-setup` спросит модели для агентов — см.
...
Теперь проект готов. Используйте `/maestro-init` для фич/багфиксов (см.
...
- `docs/project-context.md` может уже существовать → `/maestro-setup` предложит
  перечитать его (а не опрашивать заново) или пересоздать.
- Конфигурации maestro может не быть → `/maestro-setup` создаст её **идемпотентно**:
  diff-merge, **не перезаписывая** пользовательские правки.
- Модели агентов — подсказки берутся из **текущего** `.opencode/opencode.json` или
  глобального конфига (см. раздел про модели).

### Шаг 1: `/maestro-setup`

Запустите `/maestro-setup`. Поведение:
...
3. **Модели агентов** — для каждого из 7 агентов предложение формируется из:
   1. текущий `.opencode/opencode.json` (project);
...
`/maestro-design` проверяет, выполнялся ли `/maestro-setup` (Признаки:
...
Проект готов. Используйте `/maestro-init`.
...
- **Tier** — класс ответственности агента (тип задачи, модальность, требование к
  рассуждению). «Мощность» — лишь косвенное следствие (opus ≥ sonnet ≥ haiku по
  способности к рассуждению), не единственный критерий: `fable` — модальность
  (примеры/метафоры), `sanitizer` — доверие, не мощность. Контекст задаёт
  конкретная модель, не тир.
...
### Tier → тип задач / ответственность → сабагент
...
> **Тир `opus` и агент `opus` — совпадение имён.** Тир — класс задач; агент `opus` —
> конкретный сабагент этого тира (spec review). Не путать.

> **«Своя модель»** (`sanitizer`) — вне четырёх тиров, подбирается под
> security-задачу (точность, надёжность), не обязана быть мощной/лёгкой;
> `temperature 0.0` по умолчанию. Определение — в [Выборе моделей](../reference/model-selection.md).

> Без явного выбора tier сабагент наследует дорогую модель сессии — это разрушает
> экономику tier-выбора. Всегда диспатчить сабагента нужного тира.

### Как `/maestro-setup` определяет доступные модели (D2)

Список кандидатов для tier-подсказок `/maestro-setup` определяет так:

1. **Основной источник — `opencode models <provider>`** (запрос к рантайму opencode,
   не чтение глобального файла). Выполняется для каждого известного провайдера
   (`provider.*` в merge-конфиге), списки объединяются.
2. **Fallback** (если команда недоступна / ошибка / пустой список) — модели из
...
3. **Ручной ввод** — если кандидатов нет: HITL-ввод ID вручную + попытка
   `opencode models <provider>`.

> Это устраняет «странное» обращение к глобальному файлу при настройке opencode:
> доступные модели берутся из рантайма, а не «прогуливанием» конфигов вручную.

### Как выбираются модели (M1) — 7 HITL-вопросов

Для **каждого из 7 агентов** `/maestro-setup` задаёт отдельный вопрос —
выбор из вариантов через вопросный инструмент: кандидаты моделей (из D2) /
`auto` / «оставить текущую» (если агент уже настроен), плюс «свой вариант»
...
- **Учёт global-агентов.** М1 читает `agent.<name>.model` в merge-представлении:
  project `.opencode/opencode.json` → global (`~/.config/opencode/opencode.json`).
...
При `/maestro-setup` новый проект (без локальных моделей) получит «оставить
текущую (из global)» первым вариантом — наследование без вопросов. Project
`.opencode/opencode.json` переопределяет global (merge-приоритет project > global),
если нужен индивидуальный набор.
...
`temperature` — **дефолт по tier**: `/maestro-setup` проставляет его, если у агента
нет собственного значения в merge-конфиге (`.opencode/opencode.json` или global;
существующее не перезаписывает).
Полная таблица дефолтов — в [Выбор моделей](../reference/model-selection.md).

> Модели — **preference пользователя**. `/maestro-setup` лишь предлагает кандидатов
> из доступных (D2) и tier-класса; конкретный выбор — за пользователем.

### Как проверить конфигурацию моделей (тестирование)

После настройки моделей проверьте, что каждый сабагент реально диспатчится и
модель доступна. Для этого используйте команду `/test-agents`:
...
2. **Сводная таблица статусов** — по каждому агенту `OK / FAIL`:
   ...
   - **FAIL** — диспатч упал (недоступна/невалидная модель, ошибка провайдера,
     таймаут), в колонке «Причина» — текст ошибки.
...
3. **Проверка confidential-инварианта** (P1/P3, `SECURITY.md`) — доступ к
   ...
   Статусы: `PASS` (trusted прочитал, primary получил deny) / `FAIL` (утечка —
   СТОП) / `skipped` (нет `docs/confidential/**`).

**Ключевые правила:**

- Проверка **не читает конфиги** (`maestro.json`) и не сверяется с ними — валидируется
  только реальная работа модели через диспатч.
- Если trusted-модель недоступна (FAIL), confidential-инвариант воспроизводится
  только при фактической недоступности и детерминированно не тестируется
  (см. `SECURITY.md` → §6).

**Когда запускать:**

- Сразу после `/maestro-setup` — убедиться, что выбранные модели валидны и все
  7 сабагентов отвечают.
- После изменения конфигурации моделей (вручную, через `/maestro-assistant`,
  либо смены провайдера/модели) — подтвердить, что ничего не сломалось.

Подробное описание команды — в [Справочнике команд](../reference/commands.md).
...
- [Запуск первой фичи](run-first-feature.md) — полный цикл 0→18.5.
