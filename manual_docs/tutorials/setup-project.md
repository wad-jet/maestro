...
|---|---|---|
| `/maestro-init` | Setup: `project-context.md` (14 категорий), конфигурация (`maestro.json`, `.gitignore`, плагин+модели в `.opencode/opencode.json`/global), каталоги, проверки | Первый шаг — настроить проект |
...
Список кандидатов для tier-подсказок `/maestro-init` определяет так:
...
- Сразу после `/maestro-init` — убедиться, что выбранные модели валидны и все
