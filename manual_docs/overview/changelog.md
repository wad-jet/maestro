...
- **Ветка на commit-узле — только для невлитых изменений.** В `Узлы графа` поле
  `branch` выводится лишь для узлов, не влитых в mainline (tier ≠ merged); для
  merged-узлов (уже в mainline) ветка пустая — избегаем показа удалённых веток.
