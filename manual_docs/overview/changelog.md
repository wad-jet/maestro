...
- **Доставка `@maestro-benchmark`:** скилл не был зарегистрирован в каноне
  `maestro-install/agpack.yml` (skills листируются по-путно, commands —
  wholesale): все новые установки/обновления agpack получали команду без
  скилла («Skill not found»). Зарегистрирован в каноне + корневом
  `agpack.yml`; добавлена детерминированная проверка реестра
  (`maestro-install/agpack-coverage.test.mjs`, входит в `npm test`) и правило
  регистрации новых компонентов (AGENTS.md, чеклист док-синка,
  project-context §8/§9).
- **Report-guard (инверсия):** фаза `report` останавливалась на валидных
  завершённых прогонах — стоп теперь только при ОТСУТСТВИИ артефактов
  прогона; незавершённые прогоны измеряются (`finalReview: "skipped"`).
- **`$schema`-загрязнение фикстуры:** opencode авто-добавляет `"$schema"` в
  `.sandbox/.opencode/opencode.json` при старте сессии — фикстура
  «грязнилась» на каждой sandbox-сессии; `$schema` пред-встроен в
  генерируемый конфиг (авто-правка — no-op).
...
- **#95 — параллельное первое финальное ревью (4.12.0):** шаг 16 — два независимых
  ...
  порог P1.1). Регресс:
  `regression/entries/2026-09-25-parallel-first-review.md`.
...
- **Канон: утверждение короткого дизайна (Bounded) — явный HITL-гейт 7b**
  (`skills/maestro/SKILL.md`, шаг 7b + п. 3в): точка аппрува именована
  (approve/revise/cancel) и явно вынесена за пределы авто-списка 3в —
  контентный вопрос (не гейт 12: у простой фичи шаг 11 не выполняется,
  источник-условие P1.3 отсутствует); manual/auto-answer — всегда HITL,
  auto-ai — ИИ решает с доп. анализом (decision-диспатч). Авто-принятие по
  аналогии с гейтом 12 — анти-паттерн (дрейф auto-answer в auto-ai-семантику).
  Закрывает серую зону, выявленную в сессии 2026-09-21 (ретроспектива).
  Синк: `manual_docs/reference/hitl-gates.md`. Regression: LOW.

## [2026-09-21]

> **Версия 4.5.0** — Minor-релиз: секция «Таймлайн и длительности
> операций» в maestro-feedback-report (таймлайн сессии, длительности
> операций, основные задержки — по данным opencode-сессии).

### Добавлено

- **Секция «Таймлайн и длительности операций» в maestro-feedback-report:**
  сокращённый таймлайн (история) сессии и длительности отдельных операций —
  task-диспатчи по агентам (кто/сколько/сумма/макс), tools (read/write/edit/…),
  bash по командам (первое слово), топ-10 дольших операций, топ-5 пауз
  (HITL-ожидание / инференс) и вывод «что повлияло». Источник — данные
  opencode-сессии (`opencode export`), агрегация — helper `timeline.mjs`
  скилла (Node, temp-экспорт удаляется). Fallback: export/node недоступны →
  «нет данных по таймлайну», pipeline не блокируется. Regression: LOW.

## [2026-09-21]

> **Версия 4.4.0** — Minor-релиз: версионирование в project-context.md
> (поле + pipeline-хук: гейт 17 / шаг 18).

### Добавлено

- **Поле «Версионирование» в project-context.md (категория 3):** да/нет +
  схема, где живёт версия, маппинг типа изменения → уровень, действия при
  релизе (changelog-секция, синхронизация ссылок, bump-коммит). Pipeline-хук:
  шаг 0 — извлечение правил, гейт 17 — строка «Версия: X.Y.Z → A.B.C (правило)»,
  шаг 18 после merge — bump по правилам (без напоминаний). Regression: LOW.
...
- **memory_search на точках принятия решений pipeline:** канон-правило в
  ...
  (D1–D2); на шагах 11/13/16 — на усмотрение ИИ;
  guard: память не включена → молча пропуск; Spec Review (opus) — пункт
  `spec-review-prompt.md`-чек-листа (до вердикта). Ссылки/упоминания:
...
- **Процессные правила (P2.7):** отмена/прерывание диспатча ≠ отказ от ревью —
  обязательное ревью возобновляется (Anti-loop п.5); «Внеси правки»/комментарий ≠
  команда на реализацию (гейт 10, ⚑2); rollout плагина — push → agpack sync →
  очистка кэша → перезапуск (update-maestro); локальный dev-зеркал `.opencode/` —
  только полный `agpack sync` после push, частичные правки запрещены (AGENTS.md);
  переименования/метки — grep-синхронизация по всей поверхности
  (keep-docs-up-to-date.md). Regression: LOW.
...
- **Тестовая дисциплина (P2.5):** production-путь в тестах — минимум один
  интеграционный тест через production-конструктор (без ручной подстановки
  `deps.*`/логгера) на каждый публичный «шов» (закрывает wiring-gap); мягкий
  E2E-гейт на merge — прогнать E2E-критерии приёмки или явно зафиксировать
  «пропущено, риск принят» (решение HITL); инвариант → тест-гард — новый
  инвариант в SECURITY.md требует отрицательного теста в той же фиче.
  Regression: LOW.

## [2026-09-18]

> **Версия 4.2.2** — Patch-релиз: отказоустойчивость сабагентов (P2.2) + качество планов (P2.3) + эргономика HITL-гейтов (P2.4).

### Изменено

- **Эргономика HITL-гейтов (P2.4):** связанные дизайн-решения в brainstorm и
  связанные правки в revise-цикле — пакетами по 3-4 в один HITL-блок (не по
  одной за раунд); правило вопросов: свободный текст и содержательные выборы —
  обычным текстовым вопросом (не панель), вопрос-инструмент — только для
  быстрых формальных выборов. Regression: LOW.
...
> **Версия 4.2.0** — Minor-релиз: удаление `access_policy` (R3/R9 rebalance) + канон инвариантов ⚑1–4.
...
- `access_policy` из `maestro.json` и плагина `maestro-bootstrap` (R3/R9 rebalance):
  ...
  **Миграция:** после обновления перезапустите `/maestro-setup` (генерирует нативные
  ...
  Обновление локальных `.opencode/skills/*`-зеркал не требуется (`.opencode/` gitignored, регенерация при установке).

### Добавлено

- **Канон инвариантов ⚑1–4** — единая точка правды в `skills/maestro/invariants.md`
  (поставляется со скиллом в целевые приложения). SKILL.md при старте пайплайна
  ссылается на канон; маркеры ⚑ в таблицах гейтов и never-auto списках указывают
  на определения в нём. `maestro-feedback-report` фиксирует нарушения инвариантов
  ⚑1–4 (секция отчёта); команда `@maestro-feedback-report` показывает их в
  выжимке (HITL). `AGENTS.md` указывает на канон как обязательный ориентир
  при доработках.
...
- **Режимы запуска `@maestro-init`** (manual / auto-answer / auto-ai). Per-run
  флаги: `--auto-answer`/`-aa` (полный флоу — все шаги/проверки/ревью
  выполняются, рекомендации на рутинных гейтах 0/1/1.5/2/7/7d/9/12 принимаются
  автоматически с уведомлением `[auto-answer] шаг N: …`), `--auto-ai`/`-ai`
  (ИИ сам принимает решения с доп. анализом: сужденческие вопросы — decision-диспатч
  батчами через `skills/maestro/auto-ai-decision-prompt.md` (sonnet — средние,
  opus — сложные, эскалация sonnet→opus, консервативный fallback), механика —
  ralph loop; гейты 10/17 и чувствительные изменения — всегда вручную).
  Инварианты автоматизации ⚑1–4 действуют во всех режимах: PR/мерж (гейт 17),
  финальное ревью спеки (гейт 10, принятие — только явная команда) — всегда
  вручную; ИИ-тесты не полностью доверены; внешние API-контракты интеграций и
  основополагающие правила бизнес-логики — только явное HITL-решение (sensitivity
  guard на гейте 12). Security-гейты в auto-ai — гибрид только для risk-reducing
  дефолтов (вычистить и продолжить / запретить); риск и стоп — HITL (P8).
  Ralph loop не форсирует гейты 10/17 (жёсткие паузы); журнал решений + сводка
  на гейтах 10/17 (R1); анти-тупик deny (R2); пауза при аномальной плотности
  findings (R3); P5 — предпосылка режима (R4).
  Спека: `docs/superpowers/specs/2026-09-15-maestro-init-run-modes-design.md`,
  план: `docs/superpowers/plans/2026-09-15-maestro-init-run-modes-plan.md`.
...
- **Memory layer — гибридный поиск: FTS OR, единый пост-фильтр `min_score`,
  шапка результата.**
  - **FTS OR-матчинг (sqlite):** много-токенные запросы больше не требуют
    совпадения всех слов — OR-матчинг с bm25-ранжированием (полное совпадение
    всех слов ранжируется выше частичного).
  - **Гибридный auto-recall:** авто-вспоминание теперь использует FTS-ногу
    (masked-запрос без строк-плейсхолдеров `[confidential]`); единый код-путь
    с `memory_recall_preview`.
  - **`<redacted>`-плейсхолдер вырезается из FTS-запроса auto-recall:** инлайн
    `KEY=<redacted>` (sanitize, whole-fragment) больше не становится OR-термом,
    матчащим все записи с такой редакцией (следствие FTS OR; follow-up
    финального ревью).
  - **Guard полноты данных в `@maestro-memory-report`:** при отсутствии секции
    `Узлы графа (M):` в выводе `memory_stats_detail` (устаревший плагин) —
    предупреждение и упрощённый session-level граф, а не молча неверный формат.
  - **`merged` пересчитывается по `head`-ancestry при записи (не липкий).**
    Прежнее правило «re-summarize не сбрасывает merged» оставляло `merged=1`
    записям, чей `head` после ресаммаризации на feature-ветке не является
    предком mainline → такие записи ошибочно считались general (всегда в
    контексте recall) и показывались в отчёте как «в main». Теперь `merged`
    при каждой записи пересчитывается через `isAncestor(head, mainline)` (для
    заданного `head` монотонность сохранена; при перештамповке `head`
    идентичность записи меняется → флаг пересчитывается). См.
    `manual_docs/reference/memory.md` § Промоция.
  - **Единый пост-фильтр `min_score` после RRF-фьюжн** (все бэкенды): порог
    применяется к display-score после фьюжна, включая FTS-only хиты (display 0.5)
    — при `min_score` > 0.5 они не показываются.
  - **Guard пустых фильтров в `memory_search`:** пустой `author` и
    `date_from`/`date_to` ≤ 0 игнорируются (не отсекают выдачу).
  - **Шапка результата:** вторая строка вывода — `Найдено: N (порог min_score X,
    scope Y)`; пустой результат — «Ничего не найдено в памяти (порог min_score X,
    scope Y).» (`X` — `min_score`, `Y` — эффективный scope, `N` — число записей
    после scope-фильтров).
